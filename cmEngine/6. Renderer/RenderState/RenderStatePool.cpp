	...
	void RenderStatePool::Initialize()
	{
		...
		// Blend State
		{
			...
			for (uint32 idx = 0; idx < (uint32)eBlendState::Count; ++idx)
			{
				...
				hr = Renderer::Ref().GetDevice()->CreateBlendState(&desc, mBlendState[idx].GetAddressOf());
		...
		// Sampler State
		{
			...
			for (uint32 idx = 0; idx < (uint32)eSamplerState::Count; ++idx)
			{
				...
				hr = Renderer::Ref().GetDevice()->CreateSamplerState(&desc, mSamplerState[idx].GetAddressOf());
				DX_ASSERT(hr, std::format("{} create Fail", ToString(state)));
			}
		}
		...
		// Depth Stencil State
		{
			...
			for (uint32 idx = 0; idx < (uint32)eDepthStencilState::Count; ++idx)
			{
				...
				hr = Renderer::Ref().GetDevice()->CreateDepthStencilState(&desc, mDepthStencilState[idx].GetAddressOf());
				DX_ASSERT(hr, std::format("{} create Fail", ToString(state)));
			}
		}
		...
		// ReasterizerState
		{
			...
			for (uint32 idx = 0; idx < (uint32)eRasterizerState::Count; ++idx)
			{
				...
				hr = Renderer::Ref().GetDevice()->CreateRasterizerState(&desc, mRasterizeState[idx].GetAddressOf());
