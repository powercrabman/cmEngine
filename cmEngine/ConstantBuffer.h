...
namespace cmEngine
{
	...
	inline void ConstantBuffer<ConstantBufferDataType>::Create()
	{
		...
		HR hr = Renderer::GetDevice()->CreateBuffer(&desc, nullptr, mBuffer.GetAddressOf());
	...
	inline void ConstantBuffer<ConstantBufferDataType>::UpdateBuffer(const ConstantBufferDataType& inData)
	{
		...
		HR hr = Renderer::GetContext()->Map(mBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &subRes);
		...
		Renderer::GetContext()->Unmap(mBuffer.Get(), 0);
	}
}