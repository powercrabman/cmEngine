		...
		ImGuiEditCallbackHelper<Camera>(inGameEntity, "Camera##header", [&](Camera* cmr)
			{
				...
				if (cmr->mode == eCameraProjectionMode::Orthographic)
				{
					...
					{
						cmr->width = cmr->height * Renderer::Ref().GetAspectRatio();
					}
				}
				else
				{
				...
				{
					SceneManager::Ref().GetCurrentScene()->SetMainCameraEntity(inGameEntity);
				}
			});
	}
...
namespace cmEngine
{
	...
	{
		GuiRenderer::Ref().CreateConcreteGui<EditorDebugViewer>();
		GuiRenderer::Ref().CreateConcreteGui<EditorOverlay>();
		GuiRenderer::Ref().CreateConcreteGui<EditorLogViewer>();
		GuiRenderer::Ref().CreateConcreteGui<EditorSceneViewer>();
		GuiRenderer::Ref().CreateConcreteGui<EditorVoidPopup>();
		GuiRenderer::Ref().CreateConcreteGui<EditorResourceViewer>();
