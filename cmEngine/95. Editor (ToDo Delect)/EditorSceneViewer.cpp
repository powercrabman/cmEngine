...
namespace cmEngine
{
	...
	void EditorSceneViewer::RenderGui()
	{
		...
		Scene* scene = SceneManager::Ref().GetCurrentScene();
		...
		auto& reg = SceneManager::Ref().GetCurrentScene()->GetRegistry();
	...
	void EditorSceneViewer::DrawEntityEditor()
	{
		...
		if (mTargetEntity.IsValid())
		{
			...
			ImGui::SameLine();
			if (ImGui::Button("Clone")) { SceneManager::Ref().GetCurrentScene()->CloneGameEntity(mTargetEntity); };
			...
			if (ImGui::BeginPopupModal("Alert", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
			{
				...
				if (ImGui::Button("Yes"))
				{
					SceneManager::Ref().GetCurrentScene()->RemoveGameEntity(mTargetEntity);
