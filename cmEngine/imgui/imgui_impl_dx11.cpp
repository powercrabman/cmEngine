...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport* viewport)
{
    ...
    HRESULT hr = bd->pFactory->CreateSwapChain(bd->pd3dDevice, &sd, &vd->SwapChain);
    if (FAILED(hr)) {
        return;
    }
    ...
    if (vd->SwapChain)
    {
        ...
        hr = vd->SwapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
        if (SUCCEEDED(hr)) {
            hr = bd->pd3dDevice->CreateRenderTargetView(pBackBuffer, nullptr, &vd->RTView);
            pBackBuffer->Release();
        }
    }
}
